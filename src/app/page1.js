import Image from "next/image";

export default function Home() {

  const name = "juan";
  var num = 1;

  function suma(a,b){

  }

  const resta = () => {}

  suma(1,2)

  resta()

  const nums = [1,2,3,4]
  const nombres = ["juan", "pedro", "maria", "luis"];
  const personas = [{"Nombre":"Juan","Apellido":"Mendez"},{"Nombre":"Sara","Apellido":"Rey"},{"Nombre":"German","Apellido":"Gutierrez"}]
  const animales = [
    {
      "Nombre":"Emiliano",
      "Raza":"Perro",
      "Categoria":{
        "Nombre":"Perros pequeños",
        "Calificacion":1
      },
    },
    {
      "Nombre":"Bigotes",
      "Raza":"Gato",
      "Categoria":{
        "Nombre":"Gatos pequeños",
        "Calificacion":2  
      },
    },
    
    {
      "Nombre":"Pinguino",
      "Raza":"Raton",
      "Categoria":{
        "Nombre":"Roedores mas epicos",
        "Calificacion":3
      }
    }
  ]



  return (
    <div>
      <p className=" text-blue-100" >hola</p>
      <h1>hello</h1>

      <img src="data:image/png;base64,"></img>

      {num}

      <p>{1+2}</p>

      <h2>{name}</h2>

     
      {
        nums.map( ( v, i) => (
          <h1>{v}</h1>
        ) )
      }

      {
       nombres.map( ( v, i) => (
          <h2 key={i}>{i},{v}</h2>
        ) ) 
      }

      {
        personas.map((v,i) =>(
          <h1 key={i}>{v.Nombre}  {v.Apellido}</h1>
        ))
      }
      {
        animales.map((animal,i) =>(

          <h1>{animal.Nombre}, {animal.Raza}, Calificacion : {animal.Categoria.Calificacion}, {animal.Categoria.Nombre}</h1>
        ))
      }
    </div>
    // CON OBJETOS ES [{"ejemplo"},{"ejemplo2"},{"ejemplo3"}] siendo ellp
  );
}